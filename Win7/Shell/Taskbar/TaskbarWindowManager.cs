...
namespace GodLesZ.Library.Win7.Taskbar {
	internal static class TaskbarWindowManager {
		...
		private static IntPtr GrabBitmap(TaskbarWindow taskbarWindow, System.Drawing.Size requestedSize) {
			...
			if (taskbarWindow.TabbedThumbnail.WindowHandle != IntPtr.Zero) { //TabbedThumbnail is linked to WinformsControl
				if (taskbarWindow.TabbedThumbnail.CurrentHBitmap == IntPtr.Zero) {
			...
			} else if (taskbarWindow.TabbedThumbnail.WindowsControl != null) { //TabbedThumbnail is linked to a WPF UIElement
				if (taskbarWindow.TabbedThumbnail.CurrentHBitmap == IntPtr.Zero) {
