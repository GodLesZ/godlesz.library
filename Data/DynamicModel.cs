...
namespace GodLesZ.Library.Data {
	...
	public class DynamicModel : DynamicObject {
		...
		protected readonly DbProviderFactory mFactory;
		protected IEnumerable<dynamic> mSchema;
		...
		public IEnumerable<dynamic> Schema {
			get {
				if (mSchema == null) {
					mSchema = Query("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = @0", TableName);
				}
				return mSchema;
			}
		}
		...
		public DynamicModel(string connectionStringName, string tableName = "", string primaryKeyField = "", string descriptorField = "") {
			...
			mConnectionString = ConfigurationManager.ConnectionStrings[connectionStringName].ConnectionString;
		}
		...
		public virtual IEnumerable<dynamic> Query(string sql, params object[] args) {
			using (var conn = OpenConnection()) {
			    using (var cmd = CreateCommand(sql, conn, args)) {
		...
		public virtual IEnumerable<dynamic> Query(string sql, DbConnection connection, params object[] args) {
		    using (var cmd = CreateCommand(sql, connection, args)) {
		        using (var rdr = cmd.ExecuteReader()) {
		            while (rdr.Read()) {
		                yield return rdr.RecordToExpando();
		            }
		...
		private DbCommand CreateCommand(string sql, DbConnection conn, params object[] args) {
			var result = mFactory.CreateCommand();
		...
		public virtual DbConnection OpenConnection() {
			var result = mFactory.CreateConnection();
			result.ConnectionString = mConnectionString;
			result.Open();
			return result;
		}
		...
		/// </summary>
		public virtual IEnumerable<dynamic> All(string where = "", string join = "", string orderBy = "", int limit = 0, string columns = "*", params object[] args) {
			var sql = BuildSelect(join, where, orderBy, limit);
			return Query(string.Format(sql, columns, TableName), args);
		}

		private static string BuildSelect(string join, string where, string orderBy, int limit) {
